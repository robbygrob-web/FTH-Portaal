...
async def order_detail(request: Request, order_id: str, verified: bool = Depends(verify_admin_session)):
    ...
    try:
        ...
        <body>
            ...
            <div class="section chat-section" id="chatvenster" data-order-id="{order_id}">
                <h2>Berichten</h2>
            ...
            <script>
                (function() {{
                    ...
                    const orderId = chatSection.dataset.orderId;
                    ...
                    fetch('/admin/communicatie/order/' + encodeURIComponent(orderId) + '?token=' + encodeURIComponent(token))
                        ...
                        .then(data => {{
                            ...
                            data.berichten.forEach(bericht => {{
                                ...
                                // Order badge als order_id aanwezig (niet nodig hier, maar voor consistentie)
                                let orderBadgeHtml = '';
                                if (bericht.order_id && bericht.order_id !== orderId) {{
...
@router.get("/communicatie/order/{order_id}")
async def communicatie_order(
    request: Request,
    order_id: str,
    verified: bool = Depends(verify_admin_session)
):
    """Haal alle berichten op voor een specifieke order"""
    conn = None
    try:
        database_url = get_database_url()
        conn = psycopg2.connect(database_url)
        ...
        # Haal order op om klant_id te krijgen
        cur.execute("""
            SELECT klant_id 
            FROM orders 
            WHERE id = %s
        ...
        # Haal alle berichten op voor deze order
        cur.execute("""
            SELECT
                ml.id,
                ml.richting,
        ...
        return JSONResponse({
            "order_id": order_id,
            "klant_id": str(klant_id) if klant_id else None,
        ...
        _LOG.error(f"Fout bij ophalen order berichten: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            cur.close()
