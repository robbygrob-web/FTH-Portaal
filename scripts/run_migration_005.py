...
Voegt kolommen toe aan orders en facturen tabellen.
"""
import os
import psycopg2
...
def run_migration_005():
    ...
    try:
        ...
        # Facturen: mollie_payment_id
        print("[MIGRATION 005] Voeg mollie_payment_id toe aan facturen...")
        cur.execute("ALTER TABLE facturen ADD COLUMN IF NOT EXISTS mollie_payment_id VARCHAR(100);")
        conn.commit()
        print("[MIGRATION 005] ✓ mollie_payment_id toegevoegd aan facturen")
        
        # Facturen: mollie_checkout_url
        print("[MIGRATION 005] Voeg mollie_checkout_url toe aan facturen...")
        cur.execute("ALTER TABLE facturen ADD COLUMN IF NOT EXISTS mollie_checkout_url TEXT;")
        conn.commit()
        print("[MIGRATION 005] ✓ mollie_checkout_url toegevoegd aan facturen")
