...
def fetch_product_by_name(object_proxy, db, uid, password, product_name):
    """Haal een product op op basis van naam"""
    ...
    if not product_ids:
        print(f"[WAARSCHUWING] Product '{product_name}' niet gevonden")
        return None
    
    # Haal alle velden op (zonder field filter om alles te krijgen)
    # Eerst proberen zonder fields om alle velden te krijgen
    try:
        ...
        # Als read zonder fields niet werkt, probeer dan met een brede field lijst
        print(f"[INFO] Proberen met standaard velden...")
        products = object_proxy.execute_kw(
            db, uid, password,
            'product.product',
            'read',
...
def analyze_product_fields(product):
    ...
    for key, value in product.items():
        if is_value_filled(value):
            # Converteer complexe types naar strings voor weergave
            if isinstance(value, list):
                if len(value) > 0:
                    # Odoo relaties zijn vaak [id, name] tuples
...
def main():
    """Hoofdfunctie"""
    print("=" * 80)
    print("ODOO PRODUCTEN OPHALEN")
    print("=" * 80)
    
    # Producten die we willen ophalen
    product_names = [
        "Puntzak Verse Friet",
        "Verse Friet & Snacks (onbeperkt)",
        "Kids Pakket (tot 10 jaar)"
    ]
    ...
    try:
        ...
        for idx, product_name in enumerate(product_names, 1):
            print(f"\n{idx}. Ophalen: {product_name}")
            product = fetch_product_by_name(object_proxy, db, uid, password, product_name)
            
            if product:
                print(f"   [OK] Product gevonden (ID: {product.get('id', 'N/A')})")
        ...
        output_file = project_root / 'scripts' / 'odoo_products_raw.json'
        output_data = {
